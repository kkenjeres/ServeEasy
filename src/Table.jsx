        ...
        const updatedTableData = await Promise.all(
          tableData.map(async (table) => {
            const querySnapshot = await getDocs(
              collection(db, `tables/${table.id}/items`)
            );
        ...
        {sortedData.map((table, index) => {
          ...
          return (
            <article
              ...
              className={`cursor-pointer cover flex items-center  ${
                isLargeTable ? "md:col-span-2 h-[300px] mb-10" : "h-[150px]"
              } shadow-lg cursor-pointer rounded-lg ${table.color}`}
            >
                ...
                <div className="flex flex-col items-center">
                  <img
                    ...
                    className={`w-[50px] flex ${
                      isLargeTable ? "md:w-[200px] " : "md:w-[100px]"
                    }`}
                  />
                  <p className="text-center text-[16px]">
                    {"Tisch" + " " + "#" + table.id}
                  </p>
                  <p className="text-[10px] text-black bg-yellow-300 px-2 rounded-lg">
                    {table.text}
                  </p>
                </div>
                <div>
