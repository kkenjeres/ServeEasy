...
const Zahlen = ({ id, setTableData, tableId, onClose }) => {
  ...
  const handleClearTableClick = async () => {
    try {
      ...
      const currentTableItems = tableItemsSnapshot.docs.map((doc) =>
        doc.data()
      );

      // Remove all items from the current table and move items with boss: true to table 0 and items with pizza: true to table 100
      for (let item of currentTableItems) {
        await deleteDoc(
          doc(collection(db, "tables", tableId, "items"), item.id)
        );

        if (item.boss) {
          const newItem = { ...item, deletedFromTable0: true };
          await setDoc(
            doc(collection(db, "tables", "0", "items"), item.id),
            newItem
          );
        }
        if (item.pizza) {
          ...
          await setDoc(
            doc(collection(db, "tables", "1000", "items"), item.id),
            newItem
          );
        }
      }
      ...
      const tableZeroItems = tableZeroItemsSnapshot.docs.map((doc) =>
        doc.data()
      );
      ...
      const tableHundredItems = tableHundredItemsSnapshot.docs.map((doc) =>
        doc.data()
      );

      // Remove items from table 0 and table 100 that were present on the current table
      for (let item of tableZeroItems) {
        if (
          currentTableItems.some((currentItem) => currentItem.id === item.id)
        ) {
  ...
  const handlePaySelectedItems = async () => {
    ...
    for (const item of splitItems) {
      ...
      if (selectedItemQuantity < item.quantity) {
        ...
        const zeroTableItemDoc = doc(
          collection(db, "tables", "0", "items"),
          item.id.toString()
        );
        if ((await getDoc(zeroTableItemDoc)).exists()) {
          await deleteDoc(zeroTableItemDoc);
        }
        const hundredTableItemDoc = doc(
          collection(db, "tables", "1000", "items"),
          item.id.toString()
        );
  ...
  const handlePrint = async () => {
    ...
    const betragBezahltText = `Betrag bezahlt: ${(splitItems.length
      ? splitTotalPrice
      : totalPrice
    ).toFixed(2)} Euro`;
      ...
      <div className="w-full">
        <div className="bg-white px-4 py-10  flex flex-col items-center justify-center">
          <div className="flex flex-col text-center mb-10 w-full">
            <p
              ref={tableIdRef}
              className="text-black text-[24px] w-full tex-left mb-4 text-left"
            >
              Tisch: {tableId}
            </p>
            <div className="w-full flex flex-col items-center">
              <div className="w-full">
                      ...
                      <React.Fragment key={index}>
                        <tr
                          className={`text-black text-left ${
                            splitItems.some((i) => i.id === item.id)
                              ? "line-through"
                              : ""
                          }`}
                ...
                <div className="flex font-[500]">
                  <p
                    ref={summedRef}
                    className="text-black text-[16px] text-center w-full"
                  >
                    Zwischensumme:{" "}
                  </p>
                      ...
                      {firebaseData
                        .reduce((acc, item) => {
                          const group = acc.find(
                            (group) => group.percent === item.percent
                          );
                          if (group) {
                            group.items.push(item);
                          } else {
            ...
            <div className="w-full my-4 flex justify-center">
              <div className="flex justify-between items-center w-full">
                ...
                <p className="text-black text-[24px]">
                  {(splitItems.length ? splitTotalPrice : totalPrice).toFixed(
                    2
                  ) + " Euro"}
                </p>
              </div>
            ...
            <button
              ...
              className="px-4 py-2 rounded-full bg-black mt-10 text-white"
            >
              Getrennt Zahlen
            </button>
            ...
            <button
              ...
              className="px-4 py-2 rounded-full bg-white border border-black mt-10 text-black"
            >
              Zurück
            </button>
          </div>
